...
public:
	...
	*/
	static  GLuint load2DTexture(const char* filename, GLint internalFormat = GL_RGB,
		...
		int loadChannels = 0, GLboolean alpha=false)
	{
		// Step1 创建并绑定纹理对象
		GLuint textureId = 0;
        ...
        imageData =stbi_load(filename,&picWidth,&picHeight,&channels,3);
	...
	*/
	static GLuint loadCubeMapTexture(std::vector<const char*> picFilePathVec, 
		...
		GLenum picDataType = GL_UNSIGNED_BYTE, int loadChannels = 0)
	{
		...
		for (std::vector<const char*>::size_type  i =0; i < picFilePathVec.size(); ++i)
		{
			int channels = 0;
			stbi_set_flip_vertically_on_load(true);
			imageData =stbi_load(picFilePathVec[i],&picWidth,&picHeight,&channels,3);
